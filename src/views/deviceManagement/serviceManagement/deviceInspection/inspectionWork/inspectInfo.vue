<template>
  <div class="registion">
            ...
            >
              ...
              >
                <template slot-scope="scope">
                  <el-button v-if="scope.row.isFinish==='0'" type="text" title="未完成" size="small" @click="updateState(scope.row)">
              ...
              <el-table-column label="操作" width="200">
                <template slot-scope="scope">
                  <el-button type="text" title="添加明细" size="small" @click="inspectInfo(scope.row)"><i class="iconfont icontianjiaruzhishiku" /></el-button>
    ...
    <el-dialog v-if="addDetailVisible" v-model="addDetailVisible" :title="titleName" :close-on-click-modal="false" :visible.sync="addDetailVisible" class="deviceAdd newAddDit" width="700px">
      <addDetail :id="detailId" @addclose="addclose" />
    </el-dialog>
  </div>
</template>
<script>
import common from '@/utils/common'
import { selectTaskDetail, patrolTaskExecute } from '@/api/projectManage'
...
export default {
  ...
  methods: {
    ...
    updateState(row) {
      ...
      patrolTaskExecute(param).then(res => {
        if (res.success) {
          this.$message({
            showClose: true,
            message: res.msg,
