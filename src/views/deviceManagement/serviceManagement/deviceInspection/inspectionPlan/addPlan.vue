            ...
            <el-row style="margin: 0 56px;">
              <el-col :span="4">
                <el-form-item label="所属隧道：" prop="tunId">
                  <el-select v-model="conForm.tunId" popper-class="select-option" disabled placeholder="请选择所属隧道">
                    <el-option v-for="item in tunIdList" :key="item.code" :value="item.code" :label="item.name" />
                  </el-select>
                </el-form-item>
              </el-col>
              <el-col :span="4" :offset="2">
                <el-form-item label="计划名称：" prop="name" :rules="filter_rules({required:true, maxLength:30, name: '计划名称'})">
                  <el-autocomplete
            ...
            <el-row style="margin: 0 56px;">
              ...
              <el-col :span="4" :offset="3">
                <el-form-item label="巡检频次：" prop="frequency" class="unitClass">
                  <el-input-number v-model="conForm.frequency" :min="1" :max="999" class="inspectNumber" />
                  <el-select v-model="conForm.frequencyUnit" placeholder="请选择频次单位" popper-class="select-option">
...
import { insertPatrolEdit, selectPatrolName } from '@/api/projectManage'
...
export default {
  ...
  methods: {
    ...
    loadCode() {
      ...
      common.getCode({ connType: 'XJJH', num: 1 }).then(res => {
        if (res.success) {
    ...
    addDeviceIn() {
      ...
      this.tunIds = this.$route.query.tunId
        ...
        if (valid) {
          if (this.tableData.length > 0) {
            if (this.judgeLength(this.conForm)) {
              ...
              insertPatrolEdit(param).then(res => {
                if (res.success) {
                  this.$message({
                    showClose: true,
                    message: res.msg,
