    ...
    <el-tabs v-model="activeName" stretch>
      ...
      <el-tab-pane label="所属数据域" name="2" :disabled="disabled">
        <div>
          ...
          <div class="dialog-footer nextfooter">
            <!-- <el-button v-preventReClick="3000" size="small" class="button-cancel" @click.native="prev(1)">上一步</el-button> -->
            <el-button v-preventReClick="2000" size="small" class="button-sub" @click.native="submitForm">保存</el-button>
          </div>
        </div>
      </el-tab-pane>
    </el-tabs>
...
export default {
  data() {
    ...
    return {
      ...
      addFormRules: {
        ...
        personName: [{ validator: validatePass3, required: true, trigger: 'change' }],
  ...
  watch: {
    'addForm.deptId': function(newvalue, oldvalue) {
      if (newvalue !== '') {
        this.$refs.addForm.clearValidate('deptId')
      }
    }
  },
  created() {
  ...
  methods: {
    init() {
      ...
      getOrgTreeNew().then(res => {
        if (res.code === 0) {
          this.$nextTick(() => {
            this.groupTree = setTreeData(res.data)
          })
        } else {
          this.$message.error(res.msg)
        }
    ...
    handleError(e) {
      ...
      this.addForm.avatar = this.errorLoadImg //  用加载失败的图片替代之
      img.onerror = null //  清除错误:如果错误时加载时显示的图片出错，将会一直循环，所以我们必须清除掉错误，限制运行一次
    },
    prev(e) {
      this.this.activeName = e + ''
      this.active = e
